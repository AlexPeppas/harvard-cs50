...
print(f'exp values: {expo_values}')
layer_sum_per_batch = np.sum(expo_values, axis = 1, keepdims=True)
print(f'layer sum per batch: {layer_sum_per_batch}')
expo_values = expo_values / layer_sum_per_batch

print(f'softmax: {expo_values}')

expo_values_lower = np.exp(layer_outputs - np.max(layer_outputs, axis = 1, keepdims=True))
layer_sum_per_batch = np.sum(expo_values_lower, axis = 1, keepdims=True)
expo_values_lower = expo_values_lower / layer_sum_per_batch
...
print(f'softmax with max negation: {expo_values_lower}')
